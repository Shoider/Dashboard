...
export function ABCView() {
  ...
  const handleSubmit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      const ABCResponse = await axios.post<ABCResponse[]>("api2/v1/abcGet", {
        headers: {
          'Content-Type': 'application/json',
        },
      });

      if (ABCResponse.status === 200) {
        setAlert({
        ...
        setUsers(ABCResponse.data); // Actualiza el estado con los datos de la API
    ...
    <DashboardContent>
      ...
      >
        <Typography variant="h4" sx={{ flexGrow: 1 }}>
          Registros de servicios de ABC cuentas de red
        </Typography>
        <Button
          variant="contained"
          color="inherit"
            ...
            <Table sx={{ minWidth: 800 }}>
              <UserTableHead
                ...
                headLabel={[
                  ...
                  { id: 'solicitud', label: 'Tipo de Solicitud' },
                  { id: 'reporteMesa', label: '# Reporte de Mesa"' },
                  { id: 'nombreSolicitante', label: 'Nombre de Solicitante'},
                  { id: 'nombreCuenta', label: 'Nombre de Cuenta' },
                  { id: '' },
                ]}
              />
              <TableBody>
